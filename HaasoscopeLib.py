...
class Haasoscope():
    ...
    def resetchans(self):
        for chan in np.arange(HAAS_NUM_BOARD*num_chan_per_board):
            if self.gain[chan]==0:
    ...
    def setacdc(self):
        ...
        for c in range(0,4):
            realchan = (HAAS_NUM_BOARD-1-theboard)*num_chan_per_board+c
    ...
    def domarkers(self): # toggle drawing of markers, for fast ADC channels
        ...
        for line in self.lines:
            c=c+1
            if c>num_chan_per_board*HAAS_NUM_BOARD: break
            if self.drawmarkers:
                line.set_marker("o")
    ...
    def on_launch(self):
        self.xydata=np.empty([num_chan_per_board*HAAS_NUM_BOARD,2,self.num_samples-1],dtype=float)
    ...
    def on_launch_draw(self, fig_gui, ax_gui):
        plt.ion() #turn on interactive mode
        self.nlines = num_chan_per_board*HAAS_NUM_BOARD+len(max10adcchans)
        if self.db: print(("nlines=",self.nlines))
        # fig_gui, ax_gui = plt.subplots(1)
        for l in np.arange(self.nlines):
            maxchan=l-num_chan_per_board*HAAS_NUM_BOARD
            c=(0,0,0)
            if maxchan>=0: # these are the slow ADC channels
                if HAAS_NUM_BOARD>1:
                    board = int(HAAS_NUM_BOARD-1-max10adcchans[maxchan][0])
                ...
                line, = ax_gui.plot([],[], '-', label=str(max10adcchans[maxchan]), color=c, linewidth=0.5, alpha=.5)
                ...
                if HAAS_NUM_BOARD>1:
                    ...
                    if chan==2: c="blue"
                    if chan==3: c="magenta"
                line, = ax_gui.plot([],[], '-', label=self.chtext+str(l), color=c, linewidth=1.0, alpha=.9)
            self.lines.append(line)
        #for the logic analyzer
        for l in np.arange(8):
            c=(0,0,0)
            line, = ax_gui.plot([],[], '-', label="_logic"+str(l)+"_", color=c, linewidth=1.7, alpha=.65) # the leading and trailing "_"'s mean don't show in the legend
            line.set_visible(False)
            self.lines.append(line)
            if l==0: self.logicline1=len(self.lines)-1 # remember index where this first logic line is
        #other data to draw
        if self.fitline1>-1:
            line, = ax_gui.plot([],[], '-', label="fit data", color="purple", linewidth=0.5, alpha=.5)
            self.lines.append(line)
            self.fitline1=len(self.lines)-1 # remember index where this line is
        #other stuff
        if hasattr(self,'ax'): self.setxaxis(ax_gui,fig_gui)
        self.setyaxis();
        ax_gui.grid(True)
        self.vline=0
        otherline , = ax_gui.plot([self.vline, self.vline], [-2, 2], 'k--', lw=1)#,label='trigger time vert')
        self.otherlines.append(otherline)
        self.hline = 0
        otherline , = ax_gui.plot( [-2, 2], [self.hline, self.hline], 'k--', lw=1)#,label='trigger thresh horiz')
        self.otherlines.append(otherline)
        self.hline2 = 0
        otherline , = ax_gui.plot( [-2, 2], [self.hline2, self.hline2], 'k--', lw=1, color='blue')#, label='trigger2 thresh horiz')
        otherline.set_visible(False)
        self.otherlines.append(otherline)
        if self.db: print(("drew lines in launch",len(self.otherlines)))
        fig_gui.canvas.mpl_connect('button_press_event', self.onclick)
        fig_gui.canvas.mpl_connect('key_press_event', self.onpress)
        fig_gui.canvas.mpl_connect('key_release_event', self.onrelease)
        fig_gui.canvas.mpl_connect('pick_event', self.onpick)
        fig_gui.canvas.mpl_connect('scroll_event', self.onscroll)
        self.leg = ax_gui.legend(loc='upper left', bbox_to_anchor=(1.02, 1),
              ncol=1, borderaxespad=0, fancybox=False, shadow=False, fontsize=10)
        self.leg.get_frame().set_alpha(0.4)
        fig_gui.subplots_adjust(right=0.76)
        fig_gui.subplots_adjust(left=.10)
        fig_gui.subplots_adjust(top=.95)
        fig_gui.subplots_adjust(bottom=.10)
        fig_gui.canvas.set_window_title('Haasoscope')
        self.lined = dict()
        channum=0
        for legline, origline in zip(self.leg.get_lines(), self.lines):
            ...
            channum+=1
        self.drawtext()
        fig_gui.canvas.mpl_connect('close_event', self.handle_main_close)
        fig_gui.canvas.draw()
    ...
    def on_running(self, theydata, board): #update data for main plot for a board
        if board<0: #hack to tell it the max10adc channel
            ...
            posi=chantodraw+HAAS_NUM_BOARD*num_chan_per_board
            ...
            for l in np.arange(num_chan_per_board): #this draws the 4 fast ADC data channels for each board
                thechan=l+(HAAS_NUM_BOARD-board-1)*num_chan_per_board
    ...
    def autocalibrate(self,thechan,ydatanew):
        ...
        if gotonext:
            ...
            if self.autocalibchannel==num_chan_per_board*HAAS_NUM_BOARD:
                self.autocalibgainac=self.autocalibgainac+1
                if self.autocalibgainac==1:
                    self.autocalibchannel=0
                    for chan in range(num_chan_per_board*HAAS_NUM_BOARD):
                        self.selectedchannel=chan
                        self.setacdc()
    ...
    def plot_fft(self,bn): # pass in the board number
        ...
        chanonboardnum = HAAS_NUM_BOARD - int(self.fftchan/num_chan_per_board) - 1 # this is what board (0 -- (HAAS_NUM_BOARD-1)) we want to draw that fft channel from
    ...
    def getdata(self,board):
        ...
        if len(rslt)==self.num_bytes:
            ...
            if self.dooversample[num_chan_per_board*(HAAS_NUM_BOARD-board-1)]: self.oversample(0,2)
            if self.dooversample[num_chan_per_board*(HAAS_NUM_BOARD-board-1)+1]: self.oversample(1,3)
            if self.dooversample[num_chan_per_board*(HAAS_NUM_BOARD-board-1)]==9: self.overoversample(0,1)
            if self.average:
                ...
                if newswitchpos!=self.switchpos[board] or not self.havereadswitchdata:
                    for b in range(8):
                        if self.testBit(newswitchpos,b) != self.testBit(self.switchpos[board],b) or not self.havereadswitchdata:
                            ...
                            if b>=4:
                                thechan=b-4+(HAAS_NUM_BOARD-board-1)*num_chan_per_board
